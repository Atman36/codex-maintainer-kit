...
- `tools/tests/` — regression tests (run with `pytest tools/tests/`)
...
Backward-compatible aliases (runner + contract_registry):
...
- Tests live in `tools/tests/` — run `pytest tools/tests/` before committing tool changes.
- PR fan-out requires `gatekeeper.status=success` AND `decision="pr"` — `needs_human` blocks fan-out.
- Each pipeline run emits a `run_id`; per-run artifacts persisted under `analysis_report/<run_id>/`.
